import subprocess
#note this will only work on single end
...
illumina_read_2 = input("Input Illumina read 2, (press enter if single read): ") #will not work with second file


reads = [illumina_read_1]
if illumina_read_2.strip():
    reads.append(illumina_read_2)

fastq_files = []
def Fastq_converter():
    for fasta_file in reads:
        fastq_file = fasta_file.rsplit('.', 1)[0] + ".fastq"
...
with open("pipeline_errors.log", "a") as errlog:
    ...
    #running snippy
    p6 = subprocess.run(["snippy", "--ref", Reference, "--outdir", "snippy_out", "--cpus", "4", "--se",fastq_files[0]], check=True,
                        stderr=errlog)
...
#setting up to use bcftools merge
    with open("bcf_sorted.vcf.gz", "wb") as varbcf, open("snippy_sorted.vcf.gz", "wb") as varsnip:
        subprocess.run(["bgzip", "-c", "variant_caller_bcf_sorted.vcf"], stdout = varbcf, check=True, stderr=errlog)
        subprocess.run(["bgzip", "-c", "snippy_out/snps_sorted.vcf"], stdout = varsnip, check=True, stderr=errlog)
